...
class RolePlayDashboardViewModel: RoutableViewModel {
    ...
    init(router: AnyRouter, navModel: NavigationViewModel.RolePlayDashboardNavModel) {
        ...
        self.observePassData()
    }
    
    
    private func observePassData() {
        NotificationCenter.default
            .publisher(for: .RolePlay.passData)
            .receive(on: DispatchQueue.main)
            .compactMap { notification -> QuitGamePayload? in
                notification.userInfo?[Notification.RolePlayKey.passData] as? QuitGamePayload
            }
            .sink { [weak self] payload in
                guard let self else { return }
                Logger.shared.log(.info, message: "📦 passData received — chatHistory: \(payload.chatHistory.count.description) messages, images: \(payload.screenCaptureImages.count.description), videoPath: \(payload.videoFilePath?.absoluteString ?? "nil")")
                self.handleRolePlayCompletion(payload)
            }
...
extension  RolePlayDashboardViewModel {
    // MARK: - Handle Role Play Completion
    private func handleRolePlayCompletion(_ payload: QuitGamePayload) {
        guard let videoFilePath = payload.videoFilePath else {
            Logger.shared.log(.error, message: "❌ No video file path in payload")
            self.toast = Toast(style: .error, message: "Something went wrong!")
            return
        }
        
        self.selectedFileName = videoFilePath.absoluteString
        
        Logger.shared.log(.info, message: "🎬 Starting upload flow for: \(videoFilePath.lastPathComponent)")
        self.postVideoFile(fileURL: videoFilePath)
    }
}
...
extension RolePlayDashboardViewModel {
    ...
    func didTapEvaluationCriteria() {
        let evaluationParameters = navModel.evaluationParameters ?? []
...
//        )
    ...
    func didTapViewAttempts() {
        ...
        let weightageMap: [String: String] = Dictionary(
            uniqueKeysWithValues: (navModel.evaluationParameters ?? [])
    ...
    func didSelectStartRolePlay() {
        ...
        RolePlayKitService.shared.startRolePlay(router: router, projectID: projectID, courseID: courseID, moduleID: moduleID)
    }
...
//                self.loadingState = .none
    ...
    private func callPostAnalysisAPI(
        ...
        Task { [weak self] in
            ...
            moduleAttemptId =  await callMarkModuleAttempt()
